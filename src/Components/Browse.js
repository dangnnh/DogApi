...
class Browse extends Component{
    constructor(props){
        super(props);
        this.state = {value:"appenzeller"}
        this.browseImg = React.createRef()
    ...
    async getApi(apiUrl){
        let api = await fetch(apiUrl)
                        ...
                        .then(value => value)
        this.browseImg.current.src = api["message"]
    }
    handleChange(event) {
        this.setState({value: event.target.value});
    }
    componentDidMount(){
        this.getApi('https://dog.ceo/api/breed/appenzeller/images/random');
    }
    onClick(){
        let breed = this.state.value;
        let str =  `https://dog.ceo/api/breed/${breed}/images/random`
        this.getApi(str)
    }
    componentDidUpdate(){
        let breed = this.state.value;
    ...
    render(){
        return(
            ...
            <img alt="Dog" ref={this.browseImg}></img>
        </div>
        )
    }
}
